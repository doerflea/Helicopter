...
const char *WINDOWTITLE = { "OpenGL / GLUT Sample -- Anneliese Doerfler" };
...
float Unit (float v1[3], float v2[3] );
...
Display( )
{
   ...
   if(WhichView == OUTERVIEW){
     ...
     //Inside View
     gluLookAt(-0.4, 1.8, -4.9,  0. , 0. , -18.,     0., 1., 0. );
   }
  ...
  // Blade 2
   ...
   glLoadIdentity( );
   glColor3f( 1., 1., 1. );
   DoRasterString( 5., 5., 0., (char *)"Anneliese Doerfler - Animated Helicopter" );
...
InitMenus( )
{
   ...
   glutAddMenuEntry( "Outside", OUTERVIEW);
   glutAddMenuEntry( "Inside", INNERVIEW);
...
InitLists( )
{
   ...
   glRotatef( -15.,   0., 0., 1. );
   glBegin( GL_TRIANGLES );
   for( k=0, tp = Helitris; k < Helintris; k++, tp++ )
   {
      p0 = &Helipoints[ tp->p0 ];
      p1 = &Helipoints[ tp->p1 ];
      p2 = &Helipoints[ tp->p2 ];
   ...
   float x,y,z,i,j;
   for (j=0;j<M_PI; j+= M_PI/(parallel+1)){
